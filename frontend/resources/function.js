(function($) {
	"use strict";
	var HT = {}; // Khai bÃ¡o lÃ  1 Ä‘á»‘i tÆ°á»£ng
	var timer;
	...
	HT.btnSendCode = () => {
       $(document).on('click', '.btn-send', function(e){
			...
			$.ajax({
				...
				success: function(res) {
					toastr.success(res.messages, 'ThÃ´ng bÃ¡o tá»« há»‡ thá»‘ng !')
				},
			});
	   })
	}
				...
				<li>
					<a href="http://127.0.0.1:8000/${notification.canonical}.html" class="l1">
						...
						<div class="title">
							...
							<p class="time">VÃ i giÃ¢y trÆ°á»›c</p>
						</div>
					</a>
				</li>
			`;
	...
	HT.registerEvent = () => {
		$(document).on('click','.register-event', function(e){
			...
			// 	alert('Sá»± kiá»‡n Ä‘ang diá»…n ra ! Vui lÃ²ng chá» cÃ¡c sá»± kiá»‡n khÃ¡c ')
			// 	return;
			// }
			...
			if(customer_id == 0){
				...
				toastr.error('Báº¡n pháº£i Ä‘Äƒng nháº­p Ä‘á»ƒ thá»±c hiá»‡n chá»©c nÄƒng nÃ y !');
			...
			$.ajax({
				...
				success: function(res) {
					if(res.status == 1){
						_this.attr('data-status', res.status).text('ÄÃ£ Ä‘Äƒng kÃ½')
					}else{
						_this.attr('data-status', res.status).text('ÄÄƒng kÃ½ sá»± kiá»‡n')
					}
